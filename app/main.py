...
def status_label(status: str) -> str:
    return {
        "pending": "Αναμονή", "student_confirmed": "Επιβεβαίωσε μαθητής",
        "present": "Παρών", "absent": "Απών", "late": "Καθυστέρηση", "left_early": "Έφυγε νωρίς",
        "student_only_unverified": "Μόνο μαθητής / μη τελικό", "open": "Ανοιχτό", "closed": "Κλειδωμένο",
        "completed": "Ολοκληρωμένη", "selected": "Κατανεμημένος ωφελούμενος",
        "standby": "Επιλαχών", "unable_pending": "Εκκρεμεί αδυναμία",
        "unable_approved": "Εγκρίθηκε αδυναμία", "unable_rejected": "Απορρίφθηκε αδυναμία",
        "active": "Ενεργή", "removed": "Αφαιρέθηκε", "approved": "Εγκρίθηκε", "rejected": "Απορρίφθηκε",
    }.get(status, status)
...
def teaching_hours_done(classroom: Classroom) -> int:
    return sum(
        (l.teaching_hours or max(1, l.duration_minutes // 45))
        for l in classroom.lessons if l.status == "closed"
    )
...
def build_session(user: dict) -> str:
    ...
    return sign_payload({"typ": "session", "exp": exp, **user})
...
def render(request: Request, name: str, context: dict) -> HTMLResponse:
    if settings.auth_required and not current_user(request) and \
            request.url.path not in {"/login", "/auth/gov/start", "/auth/gov/callback", "/health"}:
        return RedirectResponse("/login", status_code=303)  # type: ignore[return-value]
    base = {
        "request": request, "settings": settings, "now": now_utc(),
        "current_user": current_user(request), "status_label": status_label,
        "method_label": method_label, "active_enrollments": active_enrollments,
        "active_enrollment_for": active_enrollment_for,
        "teaching_hours_done": teaching_hours_done, "completion_percent": completion_percent,
    }
    base.update(context)
...
def auth_gov_start():
    if settings.auth_provider != "gov" or not settings.gov_oauth_authorize_url or not settings.gov_oauth_client_id:
        user = {"tax_id": settings.mock_user_tax_id, "full_name": settings.mock_user_full_name,
                "role": settings.mock_user_role, "provider": "mock_taxisnet"}
        resp = RedirectResponse("/", status_code=303)
        resp.set_cookie(settings.session_cookie_name, build_session(user), httponly=True,
...
def student_link(att: Attendance) -> str:
    ...
    token = sign_payload({"typ": "student_checkin", "attendance_id": att.id,
                          "lesson_id": att.lesson_id, "student_id": att.student_id, "exp": exp})
...
def sms_body_for_attendance(att: Attendance) -> str:
    ...
    return (f"Thronos EduPresence: Το μάθημα {date_text} ξεκίνησε. "
...
def health():
    return {"ok": True, "service": settings.app_name, "environment": settings.environment,
            "l2e_enabled": settings.l2e_enabled}
...
def dashboard(request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    return render(request, "dashboard.html", {"nodes": nodes, "node_selected_counts": node_selected_counts,
                                              "pending_makeups": pending_makeups, "pending_unable": pending_unable})
...
@app.post("/nodes")
def create_node(request: Request, db: Annotated[Session, Depends(get_db)],
               name: str = Form(...), municipality: str = Form(""),
               responsible_name: str = Form(""), capacity: int = Form(15)):
    node = Node(name=name.strip(), municipality=municipality.strip() or "ΘΕΣΣΑΛΟΝΙΚΗΣ",
                responsible_name=responsible_name.strip(), capacity=capacity)
...
def node_detail(request: Request, node_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    selected = sorted([s for s in node.students if s.status in {"selected", "unable_pending", "unable_rejected"}],
                      key=lambda s: s.full_name or "")
    standby = sorted([s for s in node.students if s.status == "standby"],
                     key=lambda s: (s.priority_order, s.full_name))
    unable_requests = (db.query(UnableRequest).options(joinedload(UnableRequest.student))
                       .filter(UnableRequest.node_id == node_id)
                       .order_by(UnableRequest.created_at.desc()).all())
    return render(request, "node_detail.html", {"node": node, "selected": selected,
                                                "standby": standby, "unable_requests": unable_requests})

@app.post("/nodes/{node_id}/classes")
def create_classroom(request: Request, node_id: int, db: Annotated[Session, Depends(get_db)],
                     name: str = Form(...), capacity: int = Form(15), teacher_name: str = Form(""),
                     teacher_afm: str = Form(""), teacher_email: str = Form(""), teacher_phone: str = Form("")):
    node = db.get(Node, node_id)
    if not node:
        raise HTTPException(404, "Node not found")
    classroom = Classroom(node_id=node.id, name=name.strip(), capacity=capacity,
                          teacher_name=teacher_name.strip() or settings.mock_user_full_name,
                          teacher_afm=teacher_afm.strip(), teacher_email=teacher_email.strip(),
                          teacher_phone=teacher_phone.strip(), location=node.name,
                          l2e_tenant_id=settings.l2e_tenant_id)
    db.add(classroom); db.commit()
    write_audit(db, "classroom_created", "classroom", classroom.id, actor=actor_name(request),
                detail={"node_id": node.id, "name": classroom.name})
    return RedirectResponse(f"/nodes/{node.id}", status_code=303)

@app.post("/nodes/{node_id}/students")
def create_student(request: Request, node_id: int, db: Annotated[Session, Depends(get_db)],
                   full_name: str = Form(...), phone: str = Form(""), email: str = Form(""),
                   external_ref: str = Form(""), gender: str = Form(""), status: str = Form("selected"),
                   tax_id: str = Form(""), thr_wallet: str = Form("")):
    node = db.get(Node, node_id)
    if not node:
        raise HTTPException(404, "Node not found")
    ...
    student = Student(node_id=node.id, full_name=full_name.strip(), phone=phone.strip(),
                      email=email.strip(), external_ref=external_ref.strip(), gender=gender.strip(),
                      tax_id=tax_id.strip(), thr_wallet=thr_wallet.strip(),
                      status=status, priority_order=max_priority)
    db.add(student); db.commit()
    write_audit(db, "student_created", "student", student.id, actor=actor_name(request),
                detail={"status": status, "node_id": node.id})
    return RedirectResponse(f"/nodes/{node.id}", status_code=303)

@app.post("/nodes/{node_id}/students/{student_id}/enroll")
def enroll_student_to_class(request: Request, node_id: int, student_id: int,
                            db: Annotated[Session, Depends(get_db)], classroom_id: int = Form(...)):
    node = db.get(Node, node_id)
    student = (db.query(Student).options(joinedload(Student.enrollments))
               .filter(Student.id == student_id, Student.node_id == node_id).first())
    ...
    active_count = (db.query(Enrollment)
                    .filter(Enrollment.classroom_id == classroom.id, Enrollment.status == "active").count())
    if active_count >= classroom.capacity:
        raise HTTPException(409, "Classroom capacity reached")
    for enrollment in student.enrollments:
        if enrollment.status == "active":
            enrollment.status = "removed"; enrollment.removed_at = now_utc()
    db.add(Enrollment(classroom_id=classroom.id, student_id=student.id, status="active"))
    student.status = "selected"
    db.commit()
    write_audit(db, "student_enrolled", "classroom", classroom.id, actor=actor_name(request),
                detail={"student_id": student_id, "node_id": node_id})
    return RedirectResponse(f"/nodes/{node_id}", status_code=303)

@app.post("/nodes/{node_id}/students/{student_id}/unable/request")
def request_unable(request: Request, node_id: int, student_id: int,
                   db: Annotated[Session, Depends(get_db)],
                   reason: str = Form("Αδυναμία παρακολούθησης")):
    ...
    if student.status == "unable_pending":
        return RedirectResponse(f"/nodes/{node_id}", status_code=303)
    student.status = "unable_pending"; student.inability_reason = reason.strip()
    req = UnableRequest(node_id=node_id, student_id=student_id, reason=reason.strip(),
                        requested_by=actor_name(request))
...
def approve_unable(request: Request, node_id: int, request_id: int, db: Annotated[Session, Depends(get_db)]):
    req = (db.query(UnableRequest)
           .options(joinedload(UnableRequest.student).joinedload(Student.enrollments))
           .filter(UnableRequest.id == request_id, UnableRequest.node_id == node_id).first())
    if not req:
        raise HTTPException(404, "Unable request not found")
    req.status = "approved"; req.decided_by = actor_name(request); req.decided_at = now_utc()
    req.student.status = "unable_approved"; req.student.is_active = False
    for enrollment in req.student.enrollments:
        if enrollment.status == "active":
            enrollment.status = "removed"; enrollment.removed_at = now_utc()
    db.commit()
    write_audit(db, "unable_approved", "student", req.student_id, actor=actor_name(request),
...
def reject_unable(request: Request, node_id: int, request_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    req.student.status = "unable_rejected"; req.student.is_active = True
    db.commit()
    write_audit(db, "unable_rejected", "student", req.student_id, actor=actor_name(request),
                detail={"request_id": request_id})
    return RedirectResponse(f"/nodes/{node_id}", status_code=303)
...
def allocate_next_standby(request: Request, node_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    student = (db.query(Student)
               .filter(Student.node_id == node_id, Student.status == "standby")
               .order_by(Student.priority_order.asc(), Student.id.asc()).first())
    if not student:
        return RedirectResponse(f"/nodes/{node_id}?allocated=none", status_code=303)
    student.status = "selected"; student.is_active = True
    db.commit()
    write_audit(db, "standby_allocated", "student", student.id, actor=actor_name(request),
...
@app.post("/students")
def create_student_legacy(db: Annotated[Session, Depends(get_db)],
                          full_name: str = Form(...), phone: str = Form(""), email: str = Form("")):
    node = db.query(Node).order_by(Node.id.asc()).first()
    if not node:
        node = Node(name="Demo Κόμβος", municipality="ΘΕΣΣΑΛΟΝΙΚΗΣ")
        db.add(node); db.flush()
    student = Student(node_id=node.id, full_name=full_name.strip(),
                      phone=phone.strip(), email=email.strip(), status="selected")
    db.add(student); db.commit()
    write_audit(db, "student_created", "student", student.id, actor="teacher",
                detail={"full_name": student.full_name})
    return RedirectResponse(f"/nodes/{node.id}", status_code=303)

@app.post("/classes")
def create_classroom_legacy(db: Annotated[Session, Depends(get_db)],
                            name: str = Form(...), program_name: str = Form("Ψηφιακή Ενδυνάμωση"),
                            teacher_name: str = Form(""), location: str = Form("")):
    ...
    if not node:
        ...
        db.add(node); db.flush()
    c = Classroom(node_id=node.id, name=name.strip(), program_name=program_name.strip(),
                  teacher_name=teacher_name.strip(), location=node.name,
                  l2e_tenant_id=settings.l2e_tenant_id)
    db.add(c); db.commit()
...
def class_detail(request: Request, class_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    return render(request, "class_detail.html", {
        ...
        "students": sorted(candidates, key=lambda s: s.full_name),
...
def enroll_student(class_id: int, db: Annotated[Session, Depends(get_db)], student_id: int = Form(...)):
    ...
    exists = (db.query(Enrollment)
              .filter(Enrollment.classroom_id == class_id, Enrollment.student_id == student_id,
                      Enrollment.status == "active").first())
    if not exists:
        db.add(Enrollment(classroom_id=class_id, student_id=student_id, status="active")); db.commit()
        write_audit(db, "student_enrolled", "classroom", class_id, actor="teacher",
...
def trigger_l2e_complete(class_id: int, request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    if not classroom:
        raise HTTPException(404, "Classroom not found")
    from app.l2e_bridge import report_course_completion
    result = report_course_completion(classroom, db)
    classroom.l2e_reported_at = now_utc()
    db.commit()
...
def create_lesson(class_id: int, request: Request, db: Annotated[Session, Depends(get_db)],
                  title: str = Form("Μάθημα"), starts_at: str = Form(""),
                  duration_minutes: int = Form(120), teaching_hours: int = Form(2)):
    ...
    max_th = max(1, duration_minutes // 45)
    if teaching_hours > max_th:
        teaching_hours = max_th
    lesson = Lesson(classroom_id=class_id, title=title.strip() or "Μάθημα", starts_at=start_dt,
                    duration_minutes=duration_minutes, teaching_hours=teaching_hours,
                    created_by=classroom.teacher_name or actor_name(request))
    db.add(lesson); db.flush()
    for enrollment in classroom.enrollments:
        if enrollment.status == "active":
            db.add(Attendance(lesson_id=lesson.id, student_id=enrollment.student_id))
    db.commit()
    write_audit(db, "lesson_opened", "lesson", lesson.id, actor=lesson.created_by,
                detail={"classroom_id": class_id})
...
def lesson_detail(request: Request, lesson_id: int, db: Annotated[Session, Depends(get_db)]):
    lesson = db.query(Lesson).options(
        joinedload(Lesson.classroom).joinedload(Classroom.node),
        joinedload(Lesson.attendance_rows).joinedload(Attendance.student),
        joinedload(Lesson.attendance_rows).joinedload(Attendance.makeup),
    ...
    sms_messages = (db.query(SmsMessage).filter(SmsMessage.lesson_id == lesson.id)
                    .order_by(SmsMessage.created_at.desc()).limit(50).all())
    return render(request, "lesson_detail.html", {"lesson": lesson, "rows": rows,
                                                  "links": links, "sms_messages": sms_messages})

@app.post("/lessons/{lesson_id}/attendance/{attendance_id}/manual")
def manual_attendance(lesson_id: int, attendance_id: int, request: Request,
                      db: Annotated[Session, Depends(get_db)],
                      status: str = Form(...), reason: str = Form(""), notes: str = Form("")):
    ...
    att.status = status; att.confirmation_method = "teacher_manual"
    att.manual_reason = reason.strip(); att.notes = notes.strip()
    att.teacher_confirmed_at = now_utc(); att.finalized_at = now_utc()
    db.commit()
    write_audit(db, "attendance_manual_set", "attendance", att.id, actor=actor_name(request),
...
async def scan_student_qr(lesson_id: int, request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    if not att or att.lesson_id != lesson_id:
        raise HTTPException(404, "Attendance row not found")
    att.status = "present"; att.confirmation_method = "student_qr_teacher_scan"
    att.teacher_confirmed_at = now_utc(); att.finalized_at = now_utc()
    db.commit()
    write_audit(db, "attendance_qr_scanned", "attendance", att.id, actor=lesson.created_by,
                detail={"student_id_hash": hash_identity("student", att.student_id)})
...
def lesson_student_links(lesson_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if not lesson:
        raise HTTPException(404, "Lesson not found")
    return {"ok": True, "lesson_id": lesson.id, "links": [
        {"attendance_id": att.id, "student": att.student.full_name,
         "phone": att.student.phone, "link": student_link(att)}
...
def close_lesson(lesson_id: int, request: Request, db: Annotated[Session, Depends(get_db)]):
    lesson = db.query(Lesson).options(
        joinedload(Lesson.attendance_rows).joinedload(Attendance.student),
        joinedload(Lesson.classroom).joinedload(Classroom.lessons),
        joinedload(Lesson.classroom).joinedload(Classroom.enrollments).joinedload(Enrollment.student),
    ).filter(Lesson.id == lesson_id).first()
    if not lesson:
        raise HTTPException(404, "Lesson not found")
    assert_lesson_open(lesson)
    for att in lesson.attendance_rows:
        if att.status == "pending":
            ...
            att.status = "student_only_unverified"; att.confirmation_method = "student_self_confirmed"
            att.finalized_at = now_utc()
        payload = {
            "service": "edu_presence", "event_type": "attendance_finalized",
            ...
            "attendance_status": att.status, "confirmation_method": att.confirmation_method,
...
def student_checkin(request: Request, token: str, db: Annotated[Session, Depends(get_db)]):
    ...
    att = db.query(Attendance).options(
        ...
        joinedload(Attendance.lesson).joinedload(Lesson.classroom),
    ).filter(Attendance.id == int(payload["attendance_id"])).first()
    if not att:
    ...
    if att.status == "pending":
        att.status = "student_confirmed"; att.confirmation_method = "student_self_confirmed"
        att.student_confirmed_at = now_utc()
        db.commit()
        write_audit(db, "student_self_confirmed", "attendance", att.id, actor="student",
                    detail={"student_id_hash": hash_identity("student", att.student_id)})
...
def student_qr(token: str, db: Annotated[Session, Depends(get_db)]):
    payload = verify_token(token, expected_type="student_checkin")
    att = db.query(Attendance).options(joinedload(Attendance.lesson)).filter(
        Attendance.id == int(payload["attendance_id"])).first()
    if not att:
        raise HTTPException(404, "Attendance not found")
    if att.lesson.status != "open":
        raise HTTPException(409, "Lesson is closed")
    exp = int(time.time() + settings.qr_ttl_seconds)
    scan_token = sign_payload({"typ": "teacher_scan", "attendance_id": att.id,
                               "lesson_id": att.lesson_id, "student_id": att.student_id, "exp": exp})
    qr_data = f"THRONOS_EDUPRESENCE_SCAN:{scan_token}"
    return Response(content=qr_png_bytes(qr_data), media_type="image/png",
...
def create_makeup(attendance_id: int, request: Request, db: Annotated[Session, Depends(get_db)]):
    att = db.query(Attendance).options(
        joinedload(Attendance.lesson), joinedload(Attendance.student)
    ...
    m = Makeup(original_attendance_id=att.id, original_lesson_id=att.lesson_id,
               student_id=att.student_id, teacher_name=att.lesson.created_by)
    db.add(m); db.commit()
    write_audit(db, "makeup_created", "makeup", m.id, actor=actor_name(request),
                detail={"original_attendance_id": att.id})
...
def makeup_detail(request: Request, makeup_id: int, db: Annotated[Session, Depends(get_db)]):
    makeup = db.query(Makeup).options(
        joinedload(Makeup.student),
        joinedload(Makeup.original_lesson).joinedload(Lesson.classroom),
        joinedload(Makeup.original_attendance),
    ).filter(Makeup.id == makeup_id).first()
    if not makeup:
...
def complete_makeup(makeup_id: int, request: Request, db: Annotated[Session, Depends(get_db)],
                    makeup_date: str = Form(...), duration_minutes: int = Form(120),
                    topic: str = Form(""), student_signature_note: str = Form("Υπογράφηκε φυσικά")):
    makeup = db.query(Makeup).options(
        joinedload(Makeup.student), joinedload(Makeup.original_lesson)
    ).filter(Makeup.id == makeup_id).first()
    if not makeup:
        raise HTTPException(404, "Makeup not found")
    ...
    makeup.makeup_date = dt; makeup.duration_minutes = duration_minutes
    makeup.topic = topic.strip(); makeup.student_signature_note = student_signature_note.strip()
    makeup.status = "completed"; makeup.completed_at = now_utc()
    evidence = {
        "makeup_id": makeup.id, "original_lesson_id": makeup.original_lesson_id,
        "student_id_hash": hash_identity("student", makeup.student_id),
        "makeup_date": dt.isoformat(), "duration_minutes": duration_minutes, "topic": makeup.topic,
    }
    _, makeup.evidence_hash = canonical_hash(evidence)
    attest_payload = {
        "service": "edu_presence", "event_type": "makeup_attendance_completed",
        ...
        "makeup_date": dt.date().isoformat(), "duration_minutes": duration_minutes,
        "completed_at": makeup.completed_at.isoformat(),
    }
    attest = record_attestation(db, "makeup_attendance_completed", attest_payload)
    makeup.attestation_hash = attest.payload_hash
    db.commit()
    write_audit(db, "makeup_completed", "makeup", makeup.id, actor=actor_name(request),
                detail={"attestation_hash": makeup.attestation_hash})
    return RedirectResponse(f"/makeups/{makeup.id}/print", status_code=303)
